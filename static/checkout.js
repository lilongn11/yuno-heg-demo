...
async function initExternalCheckout(onFallback) {
  ...
  // Don't use yunoError as fallback — Apple Pay HTTP errors fire here and would
  // kill PayPal/Google Pay. Use a render timeout fallback instead.
  ...
  // Race: if startSeamlessCheckout hangs (e.g. Apple Pay error on HTTP), proceed after 3s
  ...
  // Fall back to regular checkout if no buttons render within 8 seconds
  setTimeout(() => {
    if (rendered === 0) {
